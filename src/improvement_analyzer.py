...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS improvement_opportunities (
...
def save_improvements(improvements):
    ...
    for imp in improvements:
        try:
            c.execute('''INSERT INTO improvement_opportunities 
                        (what, why, how, impact, effort, priority, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?)''',
                     (imp["what"], imp["why"], imp["how"], 
                      imp["impact"], imp["effort"], imp["priority"], now))
        except:
            pass  # Already exists
