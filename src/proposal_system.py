...
def add_criteria(name, description, weight=1):
    ...
    c = conn.cursor()
    try:
        c.execute('INSERT INTO criteria (name, description, weight) VALUES (?, ?, ?)',
...
def submit_proposal(title, description, rationale="", impact="medium", effort="medium", 
                   category="improvement", submitted_by="user"):
    ...
    c.execute('''INSERT INTO proposals 
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (title, description, rationale, impact, effort, category, submitted_by, now, now))
...
def review_proposal(proposal_id, decision, reviewer="system", score=0, comments="", criteria_scores=None):
    """Review a proposal"""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    now = datetime.now().isoformat()
    ...
    c.execute('''INSERT INTO reviews (proposal_id, reviewer, decision, score, comments, criteria, created_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?)''',
              (proposal_id, reviewer, decision, score, comments, criteria_json, now))
    
    # Update proposal status and score
    if decision == "approved":
        c.execute('UPDATE proposals SET status=?, score=?, updated_at=? WHERE id=?',
                  ('approved', score, now, proposal_id))
    elif decision == "rejected":
        c.execute('UPDATE proposals SET status=?, updated_at=? WHERE id=?',
...
def convert_to_todo(proposal_id):
    ...
    c.execute('''INSERT INTO todos (title, description, status, priority, project, created_at, updated_at)
                 VALUES (?, ?, 'todo', ?, ?, ?, ?)''',
...
def list_proposals(status=None):
    ...
    if status:
        c.execute('SELECT * FROM proposals WHERE status=? ORDER BY score DESC, created_at DESC', (status,))
    else:
...
def get_proposal_reviews(proposal_id):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT * FROM reviews WHERE proposal_id=? ORDER BY created_at DESC', (proposal_id,))
    results = c.fetchall()
    conn.close()
    return results
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("  Submit:   python proposal_system.py submit <title> <description> [rationale] [impact] [effort]")
    ...
    if cmd == "criteria":
        if len(sys.argv) > 2 and sys.argv[2] == "add":
            crit_id = add_criteria(sys.argv[3], sys.argv[4], int(sys.argv[5]) if len(sys.argv) > 5 else 1)
        ...
        if result:
            ...
            # Auto-review
            review_proposal(int(sys.argv[2]), result['decision'], "system", result['score'], 
                          f"Auto-validated: {result['score']}%", result['criteria_scores'])
