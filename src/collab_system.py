...
def start_discussion(title, created_by, context_type=None, context_id=None):
    ...
    c = conn.cursor()
    now = datetime.now().isoformat()
    c.execute('''INSERT INTO discussions (title, context_type, context_id, created_by, created_at, updated_at)
                 VALUES (?, ?, ?, ?, ?, ?)''',
...
def add_comment(discussion_id, author, content, reply_to=None):
    ...
    c.execute('''INSERT INTO comments (discussion_id, author, content, reply_to, created_at)
                 VALUES (?, ?, ?, ?, ?)''',
    ...
    c.execute('''SELECT DISTINCT author FROM comments WHERE discussion_id=? AND author!=?''',
    ...
    for user in participants:
        notify_user(user, "comment", f"{author} commented on discussion", f"discussion:{discussion_id}")
...
def close_discussion(discussion_id, user):
    ...
    c = conn.cursor()
    now = datetime.now().isoformat()
    c.execute('UPDATE discussions SET status=?, updated_at=? WHERE id=?', ('closed', now, discussion_id))
    conn.commit()
    conn.close()
...
def list_discussions(status=None, context_type=None):
    ...
    if status and context_type:
        c.execute('SELECT * FROM discussions WHERE status=? AND context_type=? ORDER BY updated_at DESC',
                  (status, context_type))
    elif status:
        c.execute('SELECT * FROM discussions WHERE status=? ORDER BY updated_at DESC', (status,))
    elif context_type:
        c.execute('SELECT * FROM discussions WHERE context_type=? ORDER BY updated_at DESC', (context_type,))
    else:
...
def get_discussion(discussion_id):
    ...
    c.execute('SELECT * FROM comments WHERE discussion_id=? ORDER BY created_at', (discussion_id,))
    comments = c.fetchall()
...
def assign_item(item_type, item_id, assigned_to, assigned_by):
    ...
    c.execute('''INSERT INTO assignments (item_type, item_id, assigned_to, assigned_by, created_at, updated_at)
                 VALUES (?, ?, ?, ?, ?, ?)''',
    ...
    # Notify assignee
    notify_user(assigned_to, "assignment", 
...
def update_assignment(assign_id, status):
    ...
    c = conn.cursor()
    now = datetime.now().isoformat()
    c.execute('UPDATE assignments SET status=?, updated_at=? WHERE id=?', (status, now, assign_id))
    conn.commit()
    conn.close()
...
def list_assignments(user=None, status=None):
    ...
    if user and status:
        c.execute('SELECT * FROM assignments WHERE assigned_to=? AND status=? ORDER BY created_at DESC',
                  (user, status))
    elif user:
        c.execute('SELECT * FROM assignments WHERE assigned_to=? ORDER BY status, created_at DESC', (user,))
    elif status:
        c.execute('SELECT * FROM assignments WHERE status=? ORDER BY created_at DESC', (status,))
    else:
...
def notify_user(user, type, message, link=None):
    ...
    c = conn.cursor()
    now = datetime.now().isoformat()
    c.execute('INSERT INTO notifications (user, type, message, link, created_at) VALUES (?, ?, ?, ?, ?)',
...
def get_notifications(user, unread_only=False):
    ...
    if unread_only:
        c.execute('SELECT * FROM notifications WHERE user=? AND read=0 ORDER BY created_at DESC', (user,))
    else:
        c.execute('SELECT * FROM notifications WHERE user=? ORDER BY created_at DESC LIMIT 50', (user,))
