...
def analyze_ideas():
    ...
    c.execute("SELECT title, source FROM ideas WHERE category='todo' AND reality_score >= 70 LIMIT 50")
    todos = c.fetchall()
...
def create_implementation_plan(strategies):
    ...
    for strategy in strategies:
        score = 0
        if strategy['priority'] == 'high': score += 3
        elif strategy['priority'] == 'medium': score += 2
        else: score += 1
        
        if strategy['impact'] == 'high': score += 3
        elif strategy['impact'] == 'medium': score += 2
        else: score += 1
        
        if strategy['effort'] == 'low': score += 3
        elif strategy['effort'] == 'medium': score += 2
        else: score += 1
...
if __name__ == "__main__":
    ...
    print(f"📊 EXTRACTED IDEAS SUMMARY")
    ...
    for i, strategy in enumerate(strategies, 1):
        ...
        print(f"   Priority: {strategy['priority'].upper()} | Impact: {strategy['impact'].upper()} | Effort: {strategy['effort'].upper()}")
