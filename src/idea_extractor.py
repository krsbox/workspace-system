...
def assess_reality(file_path, content):
    ...
    mock_patterns = [
        r'mock', r'fake', r'stub', r'dummy', r'test.*data',
        r'simulation', r'example', r'demo', r'sample'
    ]
    ...
    outdated_patterns = [
        r'deprecated', r'obsolete', r'old', r'legacy',
        r'python\s*2', r'TODO.*remove', r'FIXME.*outdated'
    ]
    ...
    if re.search(r'(test_|_test\.py|assert\s+)', str(file_path)) or \
       re.search(r'(import\s+unittest|import\s+pytest|def\s+test_)', content):
        score -= 20
...
def add_idea(source, title, description="", category="general", priority="medium", reality_score=100, warnings=""):
    ...
    c.execute('''INSERT INTO ideas 
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
        (source, title, description, category, priority, reality_score, warnings, datetime.now().isoformat()))
    idea_id = c.lastrowid
    conn.commit()
    conn.close()
    return idea_id
