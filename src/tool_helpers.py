...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS review_discussions (
    ...
    # Tool admin logs
    c.execute('''CREATE TABLE IF NOT EXISTS tool_admin_logs (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def add_discussion(review_id, review_type, user, message, parent_id=None):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT INTO review_discussions 
...
def get_discussions(review_id, review_type):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''SELECT id, user, message, parent_id, resolved, created_at
                 FROM review_discussions
                 WHERE review_id = ? AND review_type = ?
                 ORDER BY created_at''',
              (review_id, review_type))
    discussions = [{"id": r[0], "user": r[1], "message": r[2], 
                    "parent_id": r[3], "resolved": bool(r[4]), "created_at": r[5]}
                   for r in c.fetchall()]
    conn.close()
    return discussions
...
def admin_disable_tool(admin_user, tool_id, reason):
    ...
    c.execute('''INSERT INTO tool_admin_logs 
                 (admin_user, action, target_type, target_id, details, created_at)
                 VALUES (?, ?, ?, ?, ?, ?)''',
              (admin_user, 'disable_tool', 'tool', tool_id, reason, datetime.now().isoformat()))
    conn.commit()
    conn.close()
...
def get_admin_logs(limit=50):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''SELECT admin_user, action, target_type, target_id, details, created_at
                 FROM tool_admin_logs
                 ORDER BY created_at DESC
                 LIMIT ?''', (limit,))
    logs = [{"admin": r[0], "action": r[1], "target_type": r[2], 
             "target_id": r[3], "details": r[4], "created_at": r[5]}
            for r in c.fetchall()]
    conn.close()
    return logs
...
def get_tool_health_summary():
    ...
    c.execute('''SELECT AVG(CAST(success_count AS FLOAT) / NULLIF(usage_count, 0) * 100)
