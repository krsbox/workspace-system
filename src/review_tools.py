...
def init_db():
    ...
    # Review comments
    c.execute('''CREATE TABLE IF NOT EXISTS review_comments (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def start_code_review(file_path, reviewer):
    ...
    c.execute('''INSERT INTO code_reviews (file_path, reviewer, created_at, updated_at)
                 VALUES (?, ?, ?, ?)''',
...
def add_review_comment(review_id, review_type, message, line_number=None, 
                       severity="info", category="general", suggestion=None):
    """Add review comment"""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    now = datetime.now().isoformat()
    
    c.execute('''INSERT INTO review_comments 
                 (review_id, review_type, line_number, severity, category, message, suggestion, created_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
              (review_id, review_type, line_number, severity, category, message, suggestion, now))
...
def complete_review(review_id, status, score=None):
    ...
    c.execute('''UPDATE code_reviews SET status=?, score=?, findings=?, updated_at=?
                 WHERE id=?''',
...
def get_review(review_id):
    ...
    c.execute('SELECT * FROM review_comments WHERE review_id=? ORDER BY line_number, created_at',
              (review_id,))
    comments = c.fetchall()
...
def auto_review_code(file_path):
    ...
    for i, line in enumerate(lines, 1):
        if len(line) > 120:
            add_review_comment(review_id, "code", 
...
def review_proposal_quality(proposal_id):
    ...
    if len(proposal[2]) < 50:
        add_review_comment(review_id, "proposal", "Description too brief", None, "warning", "clarity",
    ...
    if not proposal[3] or len(proposal[3]) < 20:
        add_review_comment(review_id, "proposal", "Missing or weak rationale", None, "warning", "justification",
...
def create_template(name, type, checklist):
    ...
    try:
        c.execute('INSERT INTO review_templates (name, type, checklist, created_at) VALUES (?, ?, ?, ?)',
