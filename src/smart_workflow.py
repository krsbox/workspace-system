...
class WorkflowGate:
    ...
    def can_convert_proposal_to_todo(proposal_id):
        ...
        c.execute("SELECT status, title, description FROM proposals WHERE id=?", (proposal_id,))
        result = c.fetchone()
        ...
        if status not in ['approved', 'validated']:
            return False, f"Proposal not approved (status: {status}). Review required!", []
        ...
        # Check if already converted
        c.execute("SELECT id FROM todos WHERE description LIKE ?", (f"%Proposal #{proposal_id}%",))
        if c.fetchone():
    ...
    def require_discussion(proposal_id, title):
        ...
        c.execute("""INSERT INTO discussions (title, created_by, created_at, updated_at)
            VALUES (?, 1, ?, ?)""", (disc_title, now, now))
...
def convert_proposal_to_todo(proposal_id, force=False):
    ...
    c.execute("""INSERT INTO todos (title, description, status, priority, created_at, updated_at)
        VALUES (?, ?, 'todo', ?, ?, ?)""",
...
def review_proposal(proposal_id, decision, feedback=""):
    ...
    # Log review
    c.execute("""INSERT INTO reviews (proposal_id, reviewer, decision, score, comments, created_at)
        VALUES (?, 'system', ?, ?, ?, ?)""",
