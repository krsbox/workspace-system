...
if __name__ == "__main__":
    ...
    title = " ".join(sys.argv[1:-1] if len(sys.argv) > 2 and sys.argv[-1].replace('.','').isdigit() else sys.argv[1:])
    threshold = float(sys.argv[-1]) if len(sys.argv) > 2 and sys.argv[-1].replace('.','').isdigit() else 0.8
