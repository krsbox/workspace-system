...
def init_collaboration_tables():
    ...
    # Collaboration modes
    c.execute('''CREATE TABLE IF NOT EXISTS collaboration_modes (
    ...
    # Human overrides
    c.execute('''CREATE TABLE IF NOT EXISTS human_overrides (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        idea_id INTEGER,
        ai_decision TEXT,
        human_decision TEXT,
...
def save_collaboration_mode(idea_id, mode_info):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT INTO collaboration_modes 
        (idea_id, mode, confidence, requires_review, created_at)
        VALUES (?, ?, ?, ?, ?)''',
        (idea_id, mode_info['mode'], mode_info['confidence'], 
         mode_info['requires_review'], datetime.now().isoformat()))
    conn.commit()
    conn.close()
...
def save_ai_suggestion(idea_id, suggestion):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT INTO ai_suggestions
        (idea_id, suggestion_type, suggestion, confidence, reasoning, created_at)
        VALUES (?, ?, ?, ?, ?, ?)''',
        (idea_id, suggestion['type'], suggestion['action'], 
...
def get_ideas_by_mode(mode):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''SELECT i.id, i.title, i.category, i.reality_score, cm.confidence
        FROM ideas i
        JOIN collaboration_modes cm ON i.id = cm.idea_id
        WHERE cm.mode = ?
        ORDER BY cm.confidence DESC, i.reality_score DESC''', (mode,))
    ideas = c.fetchall()
    conn.close()
    return ideas
...
def human_review(idea_id, decision, notes=""):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''UPDATE collaboration_modes
...
def record_override(idea_id, ai_decision, human_decision, reason):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT INTO human_overrides
