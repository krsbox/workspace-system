...
def get_pending_suggestions(limit=20):
    ...
    c = conn.cursor()
    c.execute('''SELECT s.id, s.idea_id, i.title, s.suggestion_type, 
...
def approve_suggestion(suggestion_id, idea_id, decision, feedback=""):
    ...
    c.execute('''UPDATE ai_suggestions
        SET accepted = ?, human_feedback = ?
        WHERE id = ?''', (accepted, feedback, suggestion_id))
...
def interactive_review():
    ...
    for i, (sugg_id, idea_id, title, sugg_type, suggestion, confidence, reasoning) in enumerate(suggestions, 1):
...
def batch_approve(confidence_threshold=0.90):
    ...
    c.execute('''SELECT id, idea_id FROM ai_suggestions
        WHERE accepted IS NULL AND confidence >= ?''', (confidence_threshold,))
    suggestions = c.fetchall()
    
    for sugg_id, idea_id in suggestions:
        approve_suggestion(sugg_id, idea_id, 'approved', 
...
if __name__ == "__main__":
    ...
    if cmd == 'interactive':
        ...
        for sugg_id, idea_id, title, sugg_type, suggestion, confidence, reasoning in suggestions:
