...
try:
    ...
    from workspace_manager import (
        wiki_create, todo_add, todo_list, progress_add, progress_list
    )
    from proposal_system import (
        submit_proposal, auto_validate, convert_to_todo, list_proposals
    )
    from collab_system import (
        add_user, start_discussion, add_comment, assign_item, get_notifications
    )
    from session_manager import (
        resume_session, add_message, set_state, get_all_state, add_bookmark
    )
    from tools_manager import (
        discover_tools, execute_tool, get_tool_stats, list_tools
    )
    ...
    from prevention_system import (
        check_prevention_rules, check_guardrails, proactive_check
    )
    from maintenance_system import (
        run_due_tasks, get_complexity_score, get_utilization_summary
    )
    from tool_helpers import (
        add_discussion, get_discussions, resolve_discussion,
        admin_disable_tool, admin_enable_tool, admin_reset_tool_stats,
        get_admin_logs, get_tool_health_summary, get_review_summary
    )
...
def quick_propose(title):
    ...
    try:
        ...
        if result['decision'] == 'approved':
            ...
            print(f"→ Needs revision. Improve and resubmit.")
...
def list_todos_by_priority():
    ...
    try:
        ...
        for priority in ['urgent', 'high', 'medium', 'low']:
            ...
            if items:
                icon = "🔴" if priority == 'urgent' else "🟠" if priority == 'high' else "🟡" if priority == 'medium' else "🟢"
...
def search_all(query):
    ...
    try:
        proposals = list_proposals()
        matching = [p for p in proposals if query.lower() in p[1].lower() or query.lower() in p[2].lower()]
        if matching:
...
def main():
    ...
    if cmd == "status":
        ...
        if subcmd == "add" and len(sys.argv) >= 6:
            review_id, review_type, user, message = sys.argv[3], sys.argv[4], sys.argv[5], " ".join(sys.argv[6:])
        ...
        if subcmd == "disable" and len(sys.argv) >= 5:
            admin_user, tool_id, reason = sys.argv[3], int(sys.argv[4]), " ".join(sys.argv[5:])
            ...
            logs = get_admin_logs()
            for log in logs:
                print(f"[{log['created_at']}] {log['admin']}: {log['action']} on {log['target_type']}#{log['target_id']}")
