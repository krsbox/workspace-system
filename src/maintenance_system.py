...
def init_db():
    ...
    # Complexity metrics
    c.execute('''CREATE TABLE IF NOT EXISTS complexity_metrics (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS utilization (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def run_due_tasks():
    ...
    for task in tasks:
        ...
        # Log execution
        c.execute('''INSERT INTO task_executions (task_id, status, duration, output, error, created_at)
                     VALUES (?, ?, ?, ?, ?, ?)''',
        ...
        c.execute('''UPDATE maintenance_tasks 
...
def register_capability(name, type, complexity=1, dependencies=None):
    ...
    try:
        c.execute('''INSERT INTO capabilities (name, type, complexity, dependencies, created_at, updated_at)
                     VALUES (?, ?, ?, ?, ?, ?)''',
...
def use_capability(name):
    ...
    c.execute('UPDATE capabilities SET usage_count = usage_count + 1, updated_at=? WHERE name=?',
...
def record_complexity(component, metric_type, value, threshold=None):
    ...
    c.execute('''INSERT INTO complexity_metrics (component, metric_type, value, threshold, created_at)
                 VALUES (?, ?, ?, ?, ?)''',
...
def record_utilization(resource, used, total):
    ...
    c.execute('''INSERT INTO utilization (resource, used, total, percent, created_at)
                 VALUES (?, ?, ?, ?, ?)''',
...
def get_utilization_summary():
    ...
    c.execute('''SELECT resource, used, total, percent
                 FROM utilization
                 WHERE id IN (
