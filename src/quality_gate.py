...
def init_db():
    ...
    # Quality metrics
    c.execute('''CREATE TABLE IF NOT EXISTS quality_metrics (
    ...
    # Quality gates
    c.execute('''CREATE TABLE IF NOT EXISTS quality_gates (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    # Gate executions
    c.execute('''CREATE TABLE IF NOT EXISTS gate_executions (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    # Degradation alerts
    c.execute('''CREATE TABLE IF NOT EXISTS degradation_alerts (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        component TEXT NOT NULL,
...
def record_metric(component, metric_name, value, threshold):
    ...
    c.execute('''INSERT INTO quality_metrics (component, metric_name, value, threshold, status, created_at)
                 VALUES (?, ?, ?, ?, ?, ?)''',
...
def get_metrics(component, hours=24):
    ...
    c.execute('''SELECT * FROM quality_metrics 
...
def create_gate(name, type, rules):
    ...
    try:
        c.execute('INSERT INTO quality_gates (name, type, rules, created_at) VALUES (?, ?, ?, ?)',
...
def execute_gate(gate_name, context=None):
    ...
    for rule in rules:
        ...
        if rule_type == 'metric':
            ...
            # Get latest metric
            c.execute('''SELECT value FROM quality_metrics 
            ...
            c.execute('SELECT success_count, failure_count FROM tools WHERE name=?', (tool_name,))
            result = c.fetchone()
            ...
            c.execute('SELECT AVG(score) FROM code_reviews WHERE created_at > datetime("now", "-7 days")')
            result = c.fetchone()
    ...
    c.execute('''INSERT INTO gate_executions (gate_id, context, status, passed, failed, details, created_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?)''',
              (gate_id, context, status, passed, failed, json.dumps(details), datetime.now().isoformat()))
...
def run_assessment(type, target):
    ...
    if type == "system":
        # Check tool health
        c.execute('SELECT COUNT(*), AVG(success_count * 1.0 / NULLIF(usage_count, 0)) FROM tools WHERE status="active"')
    ...
    # Save assessment
    c.execute('''INSERT INTO assessments (type, target, score, grade, findings, recommendations, created_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?)''',
              (type, target, score, grade, json.dumps(findings), json.dumps(recommendations), datetime.now().isoformat()))
...
def check_degradation(component, metric_name, value, threshold):
    ...
    if trend == "degrading":
        ...
        c.execute('''INSERT INTO degradation_alerts (component, severity, message, metric_data, created_at)
                     VALUES (?, ?, ?, ?, ?)''',
...
def get_alerts(resolved=False):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT * FROM degradation_alerts WHERE resolved=? ORDER BY created_at DESC',
...
def resolve_alert(alert_id):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('UPDATE degradation_alerts SET resolved=1, resolved_at=? WHERE id=?',
        ...
        if subcmd == "create" and len(sys.argv) >= 5:
            ...
            if result:
                ...
                for detail in result['details']:
                    ...
                    print(f"  {status} {detail['rule'].get('type')}: {detail.get('value', detail.get('rate', 'N/A'))}")
