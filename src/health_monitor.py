...
def check_health():
    ...
    c.execute('''INSERT INTO health_checks (status, data, checked_at)
        VALUES (?, ?, ?)''', (status, data, datetime.now().isoformat()))
...
def get_health_history(hours=24):
    ...
    c.execute('''SELECT status, checked_at FROM health_checks 
        WHERE checked_at > ? ORDER BY checked_at DESC''', (since,))
