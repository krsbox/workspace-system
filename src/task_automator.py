...
def add_task(name, description, command, schedule, frequency="daily"):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT INTO auto_tasks 
...
def list_tasks():
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''SELECT id, name, schedule, frequency, run_count, enabled 
...
def setup_default_tasks():
    ...
    tasks = [
        ("Daily Backup", "Backup database daily", 
         "python3 backup_manager.py backup", "0 2 * * *", "daily"),
        ("Hourly Health Check", "Monitor system health", 
         "python3 health_monitor.py check", "0 * * * *", "hourly"),
        ("Daily Report", "Generate daily report", 
         "python3 automation_manager.py report daily", "0 9 * * *", "daily"),
        ("Weekly Cleanup", "Clean old data", 
         "python3 maintenance_system.py cleanup", "0 3 * * 0", "weekly"),
        ("Quality Check", "Run quality gates", 
         "python3 quality_gate.py check", "0 */6 * * *", "6-hourly"),
    ]
