...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS prevention_rules (
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS prevention_events (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS early_warnings (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def add_prevention_rule(name, type, condition, action, overhead="low"):
    ...
    try:
        c.execute('''INSERT INTO prevention_rules (name, type, condition, action, overhead, created_at)
                     VALUES (?, ?, ?, ?, ?, ?)''',
...
def check_prevention_rules(context):
    ...
    for rule in rules:
        ...
        if should_prevent:
            # Log prevention
            c.execute('''INSERT INTO prevention_events (rule_id, prevented, details, created_at)
                        VALUES (?, ?, ?, ?)''',
...
def add_guardrail(name, type, limit_value, enforcement="hard"):
    ...
    try:
        c.execute('''INSERT INTO guardrails (name, type, limit_value, enforcement, created_at)
                     VALUES (?, ?, ?, ?, ?)''',
...
def check_early_warning(component, metric_name, current_value, threshold):
    ...
    for level in warning_levels:
        if percent >= level:
            ...
            c.execute('''SELECT COUNT(*) FROM early_warnings 
            ...
            if c.fetchone()[0] == 0:
                ...
                c.execute('''INSERT INTO early_warnings 
                            (component, warning_type, message, threshold_percent, created_at)
                            VALUES (?, ?, ?, ?, ?)''',
                         (component, metric_name, message, level, datetime.now().isoformat()))
                conn.commit()
...
def get_warnings(acknowledged=False):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT * FROM early_warnings WHERE acknowledged=? ORDER BY created_at DESC LIMIT 20',
...
def proactive_check():
    ...
    c.execute('''SELECT name, failure_count, usage_count FROM tools 
    ...
    c.execute('SELECT COUNT(*) FROM sessions WHERE status="active" AND started_at < ?', (cutoff,))
...
if __name__ == "__main__":
    ...
    if cmd == "rule":
        ...
        if subcmd == "check" and len(sys.argv) >= 6:
            warning = check_early_warning(sys.argv[3], sys.argv[4], float(sys.argv[5]), float(sys.argv[6]))
            if warning:
