...
def init_evolution_table():
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS system_evolution (
...
def save_snapshot(metrics, notes=""):
    ...
    for metric, value in metrics.items():
        c.execute("""INSERT INTO system_evolution (timestamp, metric, value, notes)
                     VALUES (?, ?, ?, ?)""", (timestamp, metric, value, notes))
