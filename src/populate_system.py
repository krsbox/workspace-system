...
def get_high_value_ideas(limit=50):
    ...
    c = conn.cursor()
    c.execute("""SELECT i.id, i.title, i.description, i.category, i.reality_score
        FROM ideas i
...
def create_todos_from_ideas():
    ...
    # Get TODO ideas
    c.execute("""SELECT i.id, i.title, i.description, i.reality_score
        FROM ideas i
...
def consolidate_docs_to_wiki():
    ...
    for doc_file in doc_files:
        ...
        now = datetime.now().isoformat()
        c.execute("""INSERT INTO wiki (title, path, content, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?)""",
...
def review_growth():
    ...
    # Count everything
    tables = ['ideas', 'proposals', 'todos', 'discussions', 'wiki', 'knowledge', 
              'collaboration_modes', 'ai_suggestions']
