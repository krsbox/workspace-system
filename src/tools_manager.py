...
def init_db():
    ...
    # Tool improvements
    c.execute('''CREATE TABLE IF NOT EXISTS tool_improvements (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        tool_id INTEGER,
        type TEXT NOT NULL,
...
def register_tool(name, type, command, description="", category="general", version="1.0"):
    ...
    try:
        c.execute('''INSERT INTO tools (name, type, description, command, category, version, created_at, updated_at)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
...
def list_tools(category=None, status="active"):
    ...
    if category:
        c.execute('SELECT * FROM tools WHERE category=? AND status=? ORDER BY name', (category, status))
    else:
...
def update_tool_stats(tool_id, success, runtime):
    ...
    c.execute('SELECT usage_count, success_count, failure_count, avg_runtime FROM tools WHERE id=?', (tool_id,))
    stats = c.fetchone()
    ...
    c.execute('''UPDATE tools SET usage_count=?, success_count=?, failure_count=?, avg_runtime=?, updated_at=?
                 WHERE id=?''',
              (usage, success_count, failure_count, avg_runtime, datetime.now().isoformat(), tool_id))
...
def execute_tool(name, args=None, user=None):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT INTO tool_executions (tool_id, user, args, result, status, runtime, error, created_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
              (tool_id, user, args, output[:1000], 'success' if success else 'failure', runtime, error, datetime.now().isoformat()))
    conn.commit()
    conn.close()
...
def propose_improvement(tool_id, type, description, impact="medium"):
    ...
    c.execute('''INSERT INTO tool_improvements (tool_id, type, description, impact, created_at)
                 VALUES (?, ?, ?, ?, ?)''',
...
def list_improvements(status=None):
    ...
    if status:
        c.execute('''SELECT i.*, t.name FROM tool_improvements i
                     LEFT JOIN tools t ON i.tool_id = t.id
                     WHERE i.status=? ORDER BY i.created_at DESC''', (status,))
    else:
        c.execute('''SELECT i.*, t.name FROM tool_improvements i
                     LEFT JOIN tools t ON i.tool_id = t.id
...
def implement_improvement(improvement_id):
    ...
    c = conn.cursor()
    now = datetime.now().isoformat()
    c.execute('UPDATE tool_improvements SET status=?, implemented_at=? WHERE id=?',
...
def run_health_check(component, check_func):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT INTO health_checks (component, status, message, details, created_at)
                 VALUES (?, ?, ?, ?, ?)''',
...
def get_health_status():
    ...
    c.execute('''SELECT component, status, message, created_at
                 FROM health_checks
                 WHERE id IN (
...
if __name__ == "__main__":
    ...
    if module == "tool":
        if cmd == "register" and len(sys.argv) >= 6:
            tool_id = register_tool(sys.argv[3], sys.argv[4], sys.argv[5],
